...
// ES6 에서는 class로
class Brick {
    constructor(brickWidth,brickHeight,col,row,color) {
        this.left = 55 + row * (brickWidth + 10);
        this.top = 30 + col  * (brickHeight + 5);
        this.right = 55 + row * (brickWidth + 10) + brickWidth;
        this.bottom = 30 + col * (brickHeight + 5) + brickHeight;
        this.col = this.col;
        this.row = this.row;
        this.isAlive = true;
        this.color = color;
...
// 움직이는 벽돌
let brickPosX = canvas.width / 2 - brickWidth / 2;
const brickPosY = canvas.height / 2;
const brickMvDir = 1;
const brickMvSpd = 1;
...
function update() 
{
    ...
    // brick
    if(brickPosX < 0) brickMvDir = 1;
    else if(brickPosX > canvas.width - brickWidth) brickMvDir = -1;
...
function setBricks() {
    for(let i = 0; i < brickRow; i++)
    {
        ...
        for(let j =0; j < brickColumn; j++)
        {
            bricks[i][j] = new Brick(brickWidth,brickHeight,i,j,"green")
        }
    }
    // for(let i = 0; i < brickRow; i++)
    // {
...
function setMvBrick(){
    movingBrick =  new MovingBrick((brickWidth,brickHeight,i,j,"green"))
}
...
{
    context.beginPath();
    context.rect(brickPosX, brickPosY, brickWidth, brickHeight); 
